"""Focused tests for Intel Core Ultra Etap 7E (oneVPL Encoder Parity, Multi-Surface Async, Overlap)."""
...
def test_native_intel_dll_loads_and_has_7e_symbols():
    """Verify telem_intel_native.dll exports the required 7E symbols."""
...
def test_intel_proof_7e_contract_generation():
    """Verify INTEL_PROOF contract generation for INTEL_NATIVE_7E_ASYNC."""
...
def test_native_pipeline_stats_struct_layout():
    """Verify ctypes struct alignment and size."""
    stats = IntelNativePipelineStats()
    assert ctypes.sizeof(stats) > 0
    assert hasattr(stats, "total_submit_ms")
    assert hasattr(stats, "total_sync_ms")
    assert hasattr(stats, "max_pending_frames")
    assert hasattr(stats, "total_queue_wait_ms")
