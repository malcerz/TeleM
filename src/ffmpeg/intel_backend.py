...
class IntelDeviceSelection:
    """FFmpeg device arguments for the selected Intel DXGI adapter."""
...
def intel_device_selection(resolution: IntelResolution) -> IntelDeviceSelection:
    """Return the validated Intel adapter in the small FFmpeg-facing form."""
...
def intel_ffmpeg_device_args(selection: IntelDeviceSelection) -> list[str]:
    """Build explicit QSV/D3D11 device and decode-pinning arguments."""
...
def resolve_intel_force(
    ...
    *adapters*, *qsv_hw_usable* and *ffmpeg_qsv_info* are injectable so the
    ...
    log(f"[INTEL] FFmpeg QSV device: {res.qsv_device_name}")
    log(f"[INTEL] QSV child device index: {res.adapter_index}")
    log("[INTEL] QSV child device type: d3d11va")
    log("[INTEL] Decode device pinning: INTEL")
    log("[INTEL] Encode device pinning: INTEL")

    if not res.d3d11_device_ok:
        log("[INTEL] INTEL_FORCE_FAILED")
        log("[INTEL] Cross-GPU fallback: DISABLED")
        raise IntelBackendError(
            "INTEL_FORCE_FAILED: Intel D3D11 device could not be created."
        )
