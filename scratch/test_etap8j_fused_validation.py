"""ETAP 8J: Comprehensive diagnostic validation and GPU benchmarking of Fused Compositor."""
import csv
import json
import os
import subprocess
...
def run_full_validation():
    ...
    # 1. Oracle 1-pass reference
    run_single_process("etap8j_oracle_1pass", {
        "AMD_NORMALIZE_PASSES": "1",
        "AMD_FUSED_COMPOSITOR": "0",
        "AMD_DUMP_RANGE_FRAMES": "30,225,450,675,899",
    }, 901)
    for f in test_frames:
        p = root / "scratch" / f"diag_final_amf_frame_{f}.yuv"
        if p.exists():
            p.replace(root / "scratch" / f"oracle_1pass_final_{f}.yuv")

    # 2. Candidate Fused Compositor Run 1 (with frame dumps)
    run_single_process("etap8j_fused_run1", {
        "AMD_NORMALIZE_PASSES": "0",
        "AMD_FUSED_COMPOSITOR": "1",
        "AMD_DUMP_RANGE_FRAMES": "30,225,450,675,899",
    }, 901)
    for f in test_frames:
        p = root / "scratch" / f"diag_final_amf_frame_{f}.yuv"
        if p.exists():
            p.replace(root / "scratch" / f"fused_final_{f}.yuv")

    # 3. Candidate Fused Runs 2 & 3 for 3x900 stats
    run_single_process("etap8j_fused_run2", {
        "AMD_NORMALIZE_PASSES": "0",
        "AMD_FUSED_COMPOSITOR": "1",
        "AMD_DUMP_RANGE_FRAMES": "",
    }, 901)
    run_single_process("etap8j_fused_run3", {
        "AMD_NORMALIZE_PASSES": "0",
        "AMD_FUSED_COMPOSITOR": "1",
        "AMD_DUMP_RANGE_FRAMES": "",
    }, 901)
