"""ETAP 5K — micro-benchmark of the GPU_SPLIT chart CPU costs.

Measures (via the overlay profiler) the per-frame CPU cost of the split chart
path: cursor tile render, value tile render, static build (cache miss only),
and the overall compose_overlay in split mode.  Profiling of the production
A/B/C/D runs is intentionally OFF, so this gives the isolated split-render cost.
"""
...
def main() -> int:
    ...
    for idx in range(5):
        compose_overlay(canvas_w=W, canvas_h=H, layout=compose_layout, font_path=font_path,
                        _bboxes={}, gpu_capture_keys=set(CHART_KEYS), gpu_capture={},
                        split_chart_keys=set(CHART_KEYS), reuse_canvas=False, **frame_kwargs(idx))
    ...
    for idx in range(N):
        ...
        compose_overlay(canvas_w=W, canvas_h=H, layout=compose_layout, font_path=font_path,
                        ...
                        split_chart_keys=set(CHART_KEYS), reuse_canvas=False, **frame_kwargs(idx % 1131))
        profiler.finish_frame()
    total = (time.perf_counter() - started) * 1000.0 / N

    summary = profiler.summary()
    metrics = summary.get("metrics", {})
    keys = ("graph.current_cursor", "graph.dynamic_labels", "graph.final_static_build",
            "graph.background_and_chart_composite", "graph.history_chart_total",
            "compose.total")
    print(f"split compose_overlay avg: {total:.3f} ms/frame")
    for key in keys:
        if key in metrics:
            s = metrics[key]
            print(f"  {key}: avg={s.get('avg_ms', 0):.4f} ms  "
                  f"median={s.get('median_ms', 0):.4f} ms  count={s.get('count', 0)}")
    return 0
