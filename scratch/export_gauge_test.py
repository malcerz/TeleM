"""Run AMD Native export and extract 05_final_gauge_crop.png."""
import sys
import os
import subprocess
...
output_mp4 = out_dir / "gauge_test_out.mp4"
...
# Run short export (first 2 seconds = ~60 frames)
...
print(f"Export finished. Output file size: {output_mp4.stat().st_size if output_mp4.exists() else 0} bytes")

# Extract frame 30 using ffmpeg
frame_png = out_dir / "final_frame30.png"
cmd = [
    "ffmpeg", "-y", "-ss", "00:00:01", "-i", str(output_mp4),
    "-frames:v", "1", "-q:v", "2", str(frame_png),
]
subprocess.run(cmd, check=True)

# Crop gauge region: (772, 816, 324, 264)
final_img = Image.open(frame_png)
