...
import src.ffmpeg.streaming as streaming_mod
...
def main():
    ...
    out_atlas = Path('scratch/exact_parity_atlas.mp4')
    out_full = Path('scratch/exact_parity_full.mp4')
    ...
    print("\n2. Renderowanie TRUE FULL-FRAME REFERENCE (forced 1920x1080 overlay_cuda=x=0:y=0)...")
    orig_fn = streaming_mod.get_layout_hud_regions
    # Return atlas area 100% with a single region or trigger fallback
    streaming_mod.get_layout_hud_regions = lambda l, w, h, max_regions=3: (1920, 1080, [])
    try:
        stream_overlay_to_ffmpeg(
        ...
        streaming_mod.get_layout_hud_regions = orig_fn
    ...
    for name, ts in timestamps:
        p_atlas = Path(f"scratch/exact_atlas_{ts:.1f}.png")
        p_full = Path(f"scratch/exact_full_{ts:.1f}.png")
