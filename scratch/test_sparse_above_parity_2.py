...
print("=" * 90)
print("PHASE 20: 1000-FRAME BIT-FOR-BIT PARITY: SPARSE REGION COMPOSITOR vs PRODUCTION REFERENCE")
print("=" * 90)
...
for frame_idx in range(frames_to_test):
    ...
    # 1. REF: compose_overlay into 4K reusable canvas
    above_bboxes_ref = {}
    ...
    # 2. CANDIDATE: Extract directly from the crops and compare against gt
    # Test each region in regions_ref against above_full_ref
    for rx, ry, rw, rh, r_bytes in regions_ref:
        ref_patch = above_full_ref.crop((rx, ry, rx + rw, ry + rh))
        ref_bytes = ref_patch.tobytes("raw", "RGBA")
        if ref_bytes != r_bytes:
            diff_pixels_global += 1
            max_diff_global = 255
