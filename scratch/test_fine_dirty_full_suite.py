...
from src.indicators.bar import _RULER_WORKING_BUFFERS
...
print("=" * 90)
print("TESTING 2000-FRAME BIT-FOR-BIT EXACT PARITY WITH FINE DYNAMIC DIRTY RECTANGLES")
print("=" * 90)
...
for frame_idx in range(frames_to_test):
    ...
    if frame_idx == 0:
        retained_gpu_hud.paste(gt_canvas, (0, 0))
        prev_dirty_rects = []
        prev_widget_dirty = {}
        for k, box in above_bboxes.items():
        ...
        # 1. Clear previous frame's dynamic dirty regions
        for bx, by, bw, bh in prev_dirty_rects:
            retained_gpu_hud.paste((0, 0, 0, 0), (bx, by, bx + bw, by + bh))
        
        # 2. Extract and blend new frame's fine dynamic dirty regions
        current_dirty_rects = []
        for k, box in above_bboxes.items():
            ...
            if clipped is not None:
                ...
                if cw > 0 and ch > 0:
                    ...
                    # Clear target area on retained HUD and alpha composite pristine crop from ground truth
                    patch = gt_canvas.crop((cx, cy, cx + cw, cy + ch))
                    retained_gpu_hud.paste((0, 0, 0, 0), (cx, cy, cx + cw, cy + ch))
    ...
    diff = np.abs(np.asarray(gt_canvas).astype(np.int32) - np.asarray(retained_gpu_hud).astype(np.int32))
    md = int(np.max(diff))
