...
slope_cfg = v10_layout["indicators"]["slope_text"]
alt_cfg = v10_layout["indicators"]["alt_visual"]

# Measure full compose_overlay timings for baseline
...
print("=== FRESH PRODUCTION BASELINE (120 FRAMES) ===")
for key, name in [("slope_text", "Slope"), ("alt_visual", "Altitude")]:
    r = timings[key]["render"]
    p = timings[key]["paste"]
    avg_r = sum(r) / len(r)
    avg_p = sum(p) / len(p)
    avg_tot = avg_r + avg_p
    ...
    med_tot = med_r + med_p
    p95_tot = sorted([a + b for a, b in zip(r, p)])[114]
    print(f"{name:10s} | Render: {avg_r:.3f} ms (med: {med_r:.3f}) | Paste: {avg_p:.3f} ms (med: {med_p:.3f}) | TOTAL: {avg_tot:.3f} ms (med: {med_tot:.3f}, p95: {p95_tot:.3f})")

slope_tot = sum(timings["slope_text"]["render"]) / len(timings["slope_text"]["render"]) + sum(timings["slope_text"]["paste"]) / len(timings["slope_text"]["paste"])
alt_tot = sum(timings["alt_visual"]["render"]) / len(timings["alt_visual"]["render"]) + sum(timings["alt_visual"]["paste"]) / len(timings["alt_visual"]["paste"])
print(f"\nSUM (Slope + Altitude Baseline): {slope_tot + alt_tot:.3f} ms/frame")
