...
for frame_idx in range(5):
    ...
    md = int(np.max(diff))
    diff_px = int(np.sum(diff > 0) // 4)
    print(f"Frame {frame_idx}: MaxDiff = {md}, DiffPx = {diff_px}")
    if diff_px > 0:
        ys, xs = np.where(diff[:, :, 3] > 0) if np.any(diff[:, :, 3] > 0) else np.where(np.any(diff > 0, axis=-1))
        print(f"  Diff region: X=[{xs.min()}..{xs.max()}], Y=[{ys.min()}..{ys.max()}]")
        for k, box in above_bboxes.items():
            print(f"    Widget {k}: box={box}")
        break
