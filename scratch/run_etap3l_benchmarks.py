...
SCRATCH_DIR = repo_root / "scratch" / "etap3l_bench"
...
def run_benchmark_run(run_id: str, variant: str, gap_cache: str, frames: int) -> dict:
    env_clean = {
        ...
        "AMD_ABOVE_SPARSE_COMPOSE": "0",
        "AMD_CHART_GAP_CACHE": gap_cache,
    }
    ...
    fps = 30000.0 / 1001.0
    print(f"\n{'='*25} RUNNING: {run_id} ({variant}, gap_cache={gap_cache}, {frames}f) {'='*25}", flush=True)
...
def main():
    ...
    # Alternating Long A/B: 3x REF vs 3x CAND (2001 frames each)
    runs = [
        ("ref_1_long_2001f", "REF_NO_GAP_CACHE", "0", 2001),
        ("cand_1_long_2001f", "CAND_GAP_CACHE_OPT", "1", 2001),
        ("ref_2_long_2001f", "REF_NO_GAP_CACHE", "0", 2001),
        ("cand_2_long_2001f", "CAND_GAP_CACHE_OPT", "1", 2001),
        ("ref_3_long_2001f", "REF_NO_GAP_CACHE", "0", 2001),
        ("cand_3_long_2001f", "CAND_GAP_CACHE_OPT", "1", 2001),
    ]

    for run_id, variant, gap_cache, frames in runs:
        row = run_benchmark_run(run_id, variant, gap_cache, frames)
        rows.append(row)
        print(f"--> Result: {variant} | Canonical FPS = {row['canonical_fps']} | Above = {row['above_compose']} ms | Producer = {row['producer']} ms", flush=True)

    with open(BENCH_CSV, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "run", "variant", "gap_cache", "frames", "render_wall", "canonical_fps",
            "producer", "above_compose", "above_total", "consumer_native", "pipeline_total"
        ])
        writer.writeheader()
        writer.writerows(rows)

    print(f"\nAlternating Benchmark runs written to {BENCH_CSV}")
