...
SCRATCH_DIR = repo_root / "scratch" / "etap3h_bench"
...
def run_single_benchmark_3h(run_id: str, variant: str, fine_dirty: int, lean_gpu: int, frames: int) -> dict:
    env_clean = {
        ...
        "AMD_ABOVE_FINE_DIRTY": str(fine_dirty),
    }
    ...
    fps = 30000.0 / 1001.0
    print(f"\n{'='*25} RUNNING: {run_id} ({variant}, FINE_DIRTY={fine_dirty}, LEAN_GPU={lean_gpu}, {frames}f) {'='*25}", flush=True)
    ...
    rects_avg = ab_stats.get("region_count_avg", 0.0)
    bytes_avg = ab_stats.get("uploaded_bytes_avg", 0.0)

    row = {
        "run_id": run_id,
        "variant": variant,
        "fine_dirty": fine_dirty,
        "lean_gpu": lean_gpu,
...
def main():
    rows = []
    runs = [
        # Alternating Long A/B (1000 frames each)
        ("ref_1_long_1000f", "REF_FULL_WIDGET", 0, 0, 1000),
        ("cand_1_long_1000f", "CAND_FINE_DIRTY", 1, 0, 1000),
        ("ref_2_long_1000f", "REF_FULL_WIDGET", 0, 0, 1000),
        ("cand_2_long_1000f", "CAND_FINE_DIRTY", 1, 0, 1000),
        ("ref_3_long_1000f", "REF_FULL_WIDGET", 0, 0, 1000),
        ("cand_3_long_1000f", "CAND_FINE_DIRTY", 1, 0, 1000),
        # Optional GPU-Lean Check
        ("cand_gpulean_1000f", "CAND_GPULEAN", 1, 1, 1000),
    ]

    for run_id, variant, fine_dirty, lean_gpu, frames in runs:
        row = run_single_benchmark_3h(run_id, variant, fine_dirty, lean_gpu, frames)
        rows.append(row)

    with open(CSV_PATH, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "run_id", "variant", "fine_dirty", "lean_gpu", "frames", "render_wall_s", "canonical_fps",
