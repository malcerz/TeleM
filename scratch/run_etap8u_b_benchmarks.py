"""
Comprehensive Benchmark & Validation Runner for ETAP 8U-B:
Exact-Size Map Rendering + Direct 1:1 GPU Blend.
"""
import os
import sys
import json
...
out_dir_8ub = root / "Raporty" / "etap8u_b_artifacts"
out_dir_8ub.mkdir(parents=True, exist_ok=True)
...
def run_export(run_name, video_path, fit_path, frames, env_overrides=None, target_w=3840, target_h=2160):
    env = os.environ.copy()
    ...
    out_mp4 = out_dir_8ub / f"{run_name}.mp4"
    if out_mp4.exists():
        out_mp4.unlink()
    ...
    try:
        print(f"\n=======================================================", flush=True)
        print(f"BENCHMARK RUN: {run_name} ({frames}f, map_gpu_path={os.getenv('AMD_MAP_GPU_PATH')}, {target_w}x{target_h})", flush=True)
        print(f"=======================================================", flush=True)
...
def main():
    ...
    # 1. Fresh 3x 4K REFERENCE (Two-Pass Lanczos3)
    print("\n=== 1. FRESH 3x 4K REFERENCE (Two-Pass Lanczos3) ===")
    results["reference_3x"] = []
    for i in range(1, 4):
        r = run_export(f"etap8u_b_ref_run{i}", v_1131, fit_1131, 1131, env_overrides={"AMD_MAP_GPU_PATH": "REFERENCE", "AMD_MAP_FILTER": "LANCZOS"})
        results["reference_3x"].append(r)
        
    # 2. Fresh 3x 4K DIRECT (1:1 Direct Blend)
    print("\n=== 2. FRESH 3x 4K DIRECT (1:1 Direct Blend) ===")
    results["direct_3x"] = []
    for i in range(1, 4):
        r = run_export(f"etap8u_b_dir_run{i}", v_1131, fit_1131, 1131, env_overrides={"AMD_MAP_GPU_PATH": "DIRECT_AUTO"})
        results["direct_3x"].append(r)
        
    # 3. 1x Map OFF Control
    print("\n=== 3. 1x 4K MAP OFF CONTROL ===")
    r_map_off = run_export("etap8u_b_map_off", v_1131, fit_1131, 1131, env_overrides={"AMD_MAP_PATH": "CPU_REFERENCE"})
    results["map_off"] = r_map_off
    
    # 4. Full 5395-Frame Run with DIRECT
    print("\n=== 4. FULL 5395-FRAME 4K RUN (DIRECT) ===")
    r_5395 = run_export("etap8u_b_full_5395", v_5395, fit_5395, 5395, env_overrides={"AMD_MAP_GPU_PATH": "DIRECT_AUTO"})
    results["full_5395"] = r_5395
    
    out_json = out_dir_8ub / "etap8u_b_benchmark_results.json"
