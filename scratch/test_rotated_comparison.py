"""Test visual comparison with container_rotation = 180 (GoPro rotation metadata).
"""
...
def main():
    ...
    out_dir = Path("scratch/visual_compare_rot180").resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    # 1. Golden Reference (CPU / Full 4K Overlay with container_rotation = 180)
    ref_mp4 = out_dir / "ref_rot180.mp4"
    if not ref_mp4.exists() or ref_mp4.stat().st_size == 0:
        print("Rendering Golden Reference (rot180)...")
    ...
    # 2. AMD Multi-Region (rot180)
    amd_mp4 = out_dir / "amd_rot180_fixed.mp4"
    if amd_mp4.exists():
        try: amd_mp4.unlink()
        except Exception: pass

    print("Rendering AMD Multi-Region (rot180)...")
    stream_overlay_to_ffmpeg(
        ffmpeg_exe=ffmpeg_exe,
        input_files=[str(VIDEO_PATH)],
