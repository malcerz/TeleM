...
SCRATCH_DIR = repo_root / "scratch" / "etap3k_bench"
...
def run_benchmark_run(run_id: str, variant: str, frames: int) -> dict:
    env_clean = {
    ...
    fps = 30000.0 / 1001.0
    print(f"\n{'='*25} RUNNING: {run_id} ({variant}, {frames}f) {'='*25}", flush=True)
...
def main():
    ...
    # Alternating Long A/B (2001 frames each): CAND1, CAND2, CAND3 (with 3J REF for comparison)
    runs = [
        ("cand_1_long_2001f", "CAND_OPT_1", 2001),
        ("cand_2_long_2001f", "CAND_OPT_2", 2001),
        ("cand_3_long_2001f", "CAND_OPT_3", 2001),
    ]

    for run_id, variant, frames in runs:
        row = run_benchmark_run(run_id, variant, frames)
        rows.append(row)

    with open(BENCH_CSV, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "run", "variant", "frames", "render_wall", "canonical_fps",
