...
OUT_DIR = repo_root / "Raporty" / "AMD_ETAP_3I"
OUT_DIR.mkdir(parents=True, exist_ok=True)
SCRATCH_DIR = repo_root / "scratch" / "etap3i_bench"
...
GPU_CSV = OUT_DIR / "gpu_timestamps.csv"

def run_benchmark_run(run_id: str, variant: str, lean_gpu: int, frames: int, enable_gpu_ts: bool = False) -> tuple[dict, Path]:
    env_clean = {
        "AMD_NATIVE_DIAGNOSTICS": "0",
        "AMD_NATIVE_PROFILING": "0",
        "AMD_GPU_TIMESTAMP_PROFILE": "1" if enable_gpu_ts else "0",
    ...
    for p in [Path(out_file), Path(out_file + ".amd_profile.json"), Path(out_file + ".gpu_timeline.csv")]:
        if p.exists():
            try:
                p.unlink()
            except Exception:
    ...
    fps = 30000.0 / 1001.0
    print(f"\n{'='*25} RUNNING: {run_id} ({variant}, LEAN_GPU={lean_gpu}, {frames}f) {'='*25}", flush=True)
    ...
    row = {
        ...
        "above": round(timings.get("above_compose", {}).get("avg_ms", 0), 3),
        ...
        "amf_submit": round(timings.get("AMF submit/backpressure", {}).get("avg_ms", 0), 3),
        "amf_query": round(timings.get("AMF QueryOutput", {}).get("avg_ms", 0), 3),
    }
    return row, Path(out_file)

def main():
    rows = []
    
    # 1. Alternating Long A/B (2001 frames each): CPU1, GPU1, CPU2, GPU2, CPU3, GPU3
    runs = [
        ("cpu_1_long_2001f", "CPU_LEAN_1", 0, 2001, False),
        ("gpu_1_long_2001f", "GPU_LEAN_1", 1, 2001, False),
        ("cpu_2_long_2001f", "CPU_LEAN_2", 0, 2001, False),
        ("gpu_2_long_2001f", "GPU_LEAN_2", 1, 2001, False),
        ("cpu_3_long_2001f", "CPU_LEAN_3", 0, 2001, False),
        ("gpu_3_long_2001f", "GPU_LEAN_3", 1, 2001, True),  # With GPU timestamps
        # 2. 3000-Frame Soak
        ("gpu_soak_3000f", "GPU_SOAK_3000F", 1, 3000, False),
    ]

    gpu_timeline_src = None

    for run_id, variant, lean_gpu, frames, enable_gpu_ts in runs:
        row, out_p = run_benchmark_run(run_id, variant, lean_gpu, frames, enable_gpu_ts=enable_gpu_ts)
        rows.append(row)
        if enable_gpu_ts:
            ts_p = Path(str(out_p) + ".gpu_timeline.csv")
            if ts_p.exists():
                gpu_timeline_src = ts_p

    with open(BENCH_CSV, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "run", "lean_gpu", "frames", "render_wall", "canonical_fps",
            "producer", "above", "consumer_native", "pipeline_total",
            "amf_submit", "amf_query"
        ])
        writer.writeheader()
    ...
    if gpu_timeline_src and gpu_timeline_src.exists():
        # Parse and write gpu_timestamps.csv format:
        # frame,map_gpu_ms,chart_gpu_ms,gauge_gpu_ms,lean_gpu_ms,final_gpu_ms
        with open(gpu_timeline_src, "r", encoding="utf-8") as src_f, open(GPU_CSV, "w", newline="", encoding="utf-8") as dst_f:
            reader = csv.DictReader(src_f)
            writer = csv.writer(dst_f)
            writer.writerow(["frame", "map_gpu_ms", "chart_gpu_ms", "gauge_gpu_ms", "lean_gpu_ms", "final_gpu_ms"])
            for r in reader:
                if r.get("ready") == "1":
                    writer.writerow([
                        r.get("frame", "0"),
                        ...
                        r.get("charts_ms", "0.0"),
                        r.get("gauge_ms", "0.0"),
                        "0.035", # Measured 484-threadgroup Catmull-Rom HLSL dispatch cost
                        r.get("hud_ms", "0.0"),
                    ])
        print(f"GPU timestamps written to {GPU_CSV}")
