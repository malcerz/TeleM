...
def setup():
    ...
    fit = process_fit(str(ROOT / "Video" / "Popoludniowa_jazda_na_rowerze_solar_battery.fit"), video_start_dt=anchor)
    layout = _snap_nvidia_hud_layout(normalize_layout(str(ROOT / "def_layout.json"), W, H), W, H, 16)
    context = build_text_bbox_context(layout, fit_data=fit, speed_samples=speed, track_samples=track,
                                      alt_samples=alt, iso_samples=iso, exposure_samples=exposure,
                                      temperature_samples=temp)
    aw, ah, regions = get_layout_hud_regions(
        layout, W, H, max_regions=5, text_candidates=context["text_candidates"],
        phantom_keys=context["phantom_keys"],
    )
    layout["_nvidia_direct_region"] = True
    layout["_nvidia_phantom_keys"] = tuple(sorted(context["phantom_keys"]))
    layout["_nvidia_atlas_size"] = (aw, ah)
    fields = {"start_dt_utc": anchor, "speed_samples": speed, "track_samples": track,
              "alt_samples": alt, "iso_samples": iso, "exposure_samples": exposure,
              "temp_samples": temp}
    init_worker(W, H, "", layout, fields, iso_samples=iso, exposure_samples=exposure,
                temperature_samples=temp, fit_data=fit, gps_track=fit.get("track"),
                start_dt_utc=anchor, tz_offset_hours=2, speed_samples=speed,
                track_samples=track, alt_samples=alt, target_fps=FPS,
                update_rate_step=1, total_overlay_frames=N, hud_regions=regions)
    cache = build_telemetry_cache(
        layout=layout, base_dt=anchor, tz_offset_hours=2, start_dt_utc=anchor,
        speed_samples=speed, track_samples=track, alt_samples=alt, iso_samples=iso,
        exposure_samples=exposure, temperature_samples=temp, fit_data=fit,
        gps_track=fit.get("track"), chart_data=WORKER_CACHE.get("_precomputed_chart_data"),
        resolve_cache_value=_resolve_cache_value, _range_cache=WORKER_CACHE.get("_prep_cache"),
        total_frames=N, target_fps=FPS,
    )
...
def main():
    ...
    result = {"atlas": list(layout["_nvidia_atlas_size"]), "regions": [list(r) for r in regions],
              "ownership": [sorted(x) for x in (_direct_region_members(layout, regions) or [])],
              "parity": {}}
    for idx in (0, 540, 1350, 2700, 4050, 4860, 5399):
        ...
        result["parity"][str(idx)] = {"max_diff": int(diff.max()),
                                      "different_pixels": int(np.any(diff != 0, axis=2).sum()),
                                      "legacy_shape": list(a.shape), "direct_shape": list(b.shape)}
    (ROOT / "scratch" / "etap5b6_direct_validation.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
