...
def main():
    ...
    # 1. Long Baseline (2001 frames) on GX030120 / def_layout
    print("\n" + "#" * 80)
    print("PHASE 4: LONG BASELINE (2001 FRAMES, GX030120, def_layout, 4K)")
    print("#" * 80)
    prof_long = run_pipeline("long_baseline_2001f", base_layout, 2001)
    results["long_baseline_2001f"] = prof_long

    # 2. Automated Ablation Matrix (300 frames each)
    # Active CPU ABOVE widgets in def_layout.json:
    # alt_text (bar), fit_distance_text (bar), time_display (time_display),
    # iso_text (text), exposure_text (text), temp_text (text), lean_indicator (lean)
    ablation_targets = [
        "alt_text",
        "fit_distance_text",
        "time_display",
    ...
    print("\n" + "#" * 80)
    print("PHASE 10: AUTOMATED ABLATION MATRIX (300 FRAMES PER RUN)")
    print("#" * 80)

    # First: 300f FULL baseline
    prof_full_300 = run_pipeline("abl_full_300f", base_layout, 300)
    results["abl_full_300f"] = prof_full_300

    for target in ablation_targets:
        abl_layout = copy.deepcopy(base_layout)
        if target in abl_layout.get("indicators", {}):
            abl_layout["indicators"][target]["enabled"] = False
        prof_abl = run_pipeline(f"abl_off_{target}", abl_layout, 300)
        results[f"abl_off_{target}"] = prof_abl

    with open(OUT_DIR / "etap3b_audit_raw.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print("\nAudit runs completed and saved to Raporty/AMD_ETAP_3B/etap3b_audit_raw.json")
