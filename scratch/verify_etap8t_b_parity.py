"""
Pixel parity verification between SYNC and ASYNC pipeline runs in ETAP 8T-B.
"""
import subprocess
import json
import numpy as np
...
sync_mp4 = root / "Raporty" / "etap8t_b_artifacts" / "etap8t_b_sync_run1.mp4"
async_mp4 = root / "Raporty" / "etap8t_b_artifacts" / "etap8t_b_async_run1.mp4"

print(f"Comparing SYNC vs ASYNC parity on 100 frames...")

# Extract 100 frames from both videos using ffmpeg
...
if np.max(max_diffs) == 0:
    ...
    print(f"RESULT: 0 visible differences (HEVC intra/inter bitstream delta max = {np.max(max_diffs)})")
